...
public class OldXinhuaCrawler implements Crawler {
	...
	public ArrayList<News> getNews(Date date) throws IOException {
		...
		for (Element layoutBlock : layoutBlocks) {
			...
			for (Element report : reports) {
				if (report.toString().contains("javascript")) {
					continue;
				}
				...
				// System.out.println(newsdom);

				Element content = newsdom.getElementById("ozoom");
				Element title = newsdom.getElementById("contenttitle");
				Elements subtitles = newsdom.getElementsByAttributeValue(
